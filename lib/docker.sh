...
get_container_ip() {
    ...
    docker inspect -f "{{.NetworkSettings.Networks.${network_name}.IPAddress}}" "$container_name" 2>/dev/null
}
...
pull_docker_image() {
    ...
    run_sudo docker pull "$image_name"
    return $?
}
...
export_container_config() {
    ...
    docker inspect "$container_name" > "$output_file"
